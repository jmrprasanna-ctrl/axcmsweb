...
async function fetchTemplatePdfBuffer(){
    return fetchTemplatePdfBufferByType("invoice");
}

function getTemplateTypeMeta(templateType){
    const key = String(templateType || "invoice").trim().toLowerCase();
    if(key === "quotation"){
        return { endpoint: "/invoices/quotation-template-pdf", feature: "quotation", label: "Quotation" };
    }
    if(key === "quotation2"){
        return { endpoint: "/invoices/quotation-2-template-pdf", feature: "quotation2", label: "Quotation 2" };
    }
    if(key === "quotation3"){
        return { endpoint: "/invoices/quotation-3-template-pdf", feature: "quotation3", label: "Quotation 3" };
    }
    return { endpoint: "/invoices/template-pdf", feature: "invoice", label: "Invoice" };
}

async function fetchTemplatePdfBufferByType(templateType){
    const meta = getTemplateTypeMeta(templateType);
    if(!hasMappedFeature(meta.feature)){
        throw new Error(`${meta.label} function is not mapped for this user and database.`);
    }
    const token = localStorage.getItem("token");
    const res = await fetch(`${BASE_URL}${meta.endpoint}`, {
        headers: {
            "Authorization": `Bearer ${token}`
        }
    });
    if(!res.ok){
        throw new Error(`Failed to load ${meta.label} template PDF from backend.`);
    }
    return res.arrayBuffer();
}

async function ensureTemplateDataUrl(width, height, templateType="invoice"){
    const typeKey = String(templateType || "invoice").trim().toLowerCase();
    const cacheKey = `${typeKey}:${width}x${height}`;
    if(templateRenderCache[cacheKey]){
        templateDataUrl = templateRenderCache[cacheKey].dataUrl;
        templatePlacement = templateRenderCache[cacheKey].placement;
    ...
    const buffer = await fetchTemplatePdfBufferByType(typeKey);
...
async function drawInvoice(data, mimeType="image/png", renderSize=null, templateType="invoice"){
    ...
    try{
        const bgUrl = await ensureTemplateDataUrl(W, H, templateType);
    ...
    if(getLayoutVisible("customerName")){
        fitText(ctx, c.name || "", m.x(POS.customer.x + customerNameCfg.x), m.y(POS.customer.nameY - 103 + customerNameCfg.y), m.w(POS.customer.w), customerNameFont, customerNameWeight, customerNameFamily);
    }
    ...
    if(getLayoutVisible("date")){
        fitText(ctx, asDate(data.invoice.invoice_date || data.invoice.createdAt), m.x(POS.meta.dateX + 205 + dateCfg.x), m.y(POS.meta.dateY - 105 + dateCfg.y), m.w(POS.meta.w), dateFont, dateWeight, dateFamily);
    }
    ...
    data.items.slice(0, 18).forEach((item, idx) => {
        ...
        const desc = product.description || product.model || "";
        ...
        if(getLayoutVisible("description")) fitText(ctx, `${code} ${desc}`.trim(), c1 + m.w(28 + descriptionCfg.x), y + m.h(descriptionCfg.y), c2 - c1 - m.w(16), descriptionFontSize, descriptionWeight, descriptionFontFamily);
        ...
        if(getLayoutVisible("rate")) drawRightFixed(ctx, money(item.rate), c4 + m.w(12 + rateCfg.x) - m.w(150), y + m.h(rateCfg.y), rateFontSize, rateWeight, rateFontFamily);
        ...
        if(getLayoutVisible("grossAmount")) drawRight(ctx, money(item.gross), tableRight + m.w(12 + grossCfg.x) - m.w(40), y + m.h(grossCfg.y), tableRight - c5 - m.w(16), grossFontSize, grossWeight, grossFontFamily);
        y += rowH;
    });
    ...
    if(getLayoutVisible("amountWords")){
        drawFixedText(
            ctx,
            amountInWords(data.total_amount),
...
function syncInvDateInput(){
    const input = document.getElementById("invDateInput");
    if(!input || !latestInvoiceData || !latestInvoiceData.invoice) return;
    const raw = String(latestInvoiceData.invoice.invoice_date || "").trim();
    if(/^\d{4}-\d{2}-\d{2}$/.test(raw)){
        input.value = raw;
    }
}

async function updateInvoiceDateFromTile(){
    const input = document.getElementById("invDateInput");
    const invoiceId = new URLSearchParams(window.location.search).get("id");
    if(!input || !invoiceId) return;
    const value = String(input.value || "").trim();
    if(!/^\d{4}-\d{2}-\d{2}$/.test(value)){
        alert("Enter a valid Inv Date.");
        return;
    }
    try{
        const res = await request(`/invoices/${invoiceId}/payment`, "PUT", { invoice_date: value });
        if(!latestInvoiceData) latestInvoiceData = { invoice: {} };
        if(!latestInvoiceData.invoice) latestInvoiceData.invoice = {};
        latestInvoiceData.invoice.invoice_date = res?.invoice?.invoice_date || value;
        await refreshPreviewFromLatest();
        if(typeof showMessageBox === "function"){
            showMessageBox("Inv Date updated");
        }
    }catch(err){
        alert(err.message || "Failed to update Inv Date");
    }
}
...
function initSignControl(){
    ...
    if(!signChk) return;
    const allowed = hasMappedFeature("sign_c");
...
function initSignVControl(){
    ...
    signVChk.checked = !!allowed;
    signVEnabled = !!allowed;
...
function initSealControl(){
    ...
    if(!sealChk) return;
    const allowed = hasMappedFeature("seal_c");
...
function initSealVControl(){
    ...
    sealVChk.checked = !!allowed;
    sealVEnabled = !!allowed;
...
async function buildRenderedInvoicePdf(templateType="invoice", filePrefix="Invoice"){
    if(!latestInvoiceData){
        throw new Error("Invoice preview is not ready yet.");
    }
    const image = await drawInvoice(latestInvoiceData, "image/jpeg", { width: BASE_W, height: BASE_H }, templateType);
    ...
    const safePrefix = String(filePrefix || "Invoice").replace(/[\\/:*?"<>|]/g, "_");
    const fileName = `${safePrefix}_${safeInvoiceNo}_${safeCustomer}.pdf`;
...
async function emailInvoice(){
    ...
    try{
        const rendered = await buildRenderedInvoicePdf("invoice", "Invoice");
...
async function emailQuotation(){
    ...
    if(!hasMappedFeature("quotation")){
        alert("Quotation function is not mapped for this user/database.");
        return;
    }
    try{
        const rendered = await buildRenderedInvoicePdf("quotation", "Quotation");
        const attachmentDataUrl = await blobToDataUrl(rendered.pdfBlob);
        const res = await request(`/invoices/${invoiceId}/send-email`, "POST", {
            attachment_pdf_base64: attachmentDataUrl,
            attachment_file_name: rendered.fileName,

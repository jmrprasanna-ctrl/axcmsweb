...
async function fetchTemplatePdfBuffer(){
    if(!hasMappedFeature("quotation")){
        throw new Error("Quotation function is not mapped for this user and database.");
    }
    const token = localStorage.getItem("token");
    const res = await fetch(`${BASE_URL}/invoices/quotation-template-pdf`, {
        headers: {
...
function syncQutDateInput(){
    const input = document.getElementById("qutDateInput");
    if(!input || !latestInvoiceData || !latestInvoiceData.invoice) return;
    const raw = String(latestInvoiceData.invoice.quotation_date || latestInvoiceData.invoice.invoice_date || "").trim();
    if(/^\d{4}-\d{2}-\d{2}$/.test(raw)){
        input.value = raw;
    }
}

async function updateQuotationDateFromTile(){
    const input = document.getElementById("qutDateInput");
    const invoiceId = new URLSearchParams(window.location.search).get("id");
    if(!input || !invoiceId) return;
    const value = String(input.value || "").trim();
    if(!/^\d{4}-\d{2}-\d{2}$/.test(value)){
        alert("Enter a valid Qut Date.");
        return;
    }
    try{
        const res = await request(`/invoices/${invoiceId}/payment`, "PUT", { quotation_date: value });
        if(!latestInvoiceData) latestInvoiceData = { invoice: {} };
        if(!latestInvoiceData.invoice) latestInvoiceData.invoice = {};
        latestInvoiceData.invoice.quotation_date = res?.invoice?.quotation_date || value;
        await refreshPreviewFromLatest();
        if(typeof showMessageBox === "function"){
            showMessageBox("Qut Date updated");
        }
    }catch(err){
        alert(err.message || "Failed to update Qut Date");
    }
}
...
async function printPDF(){
    ...
    try{
        ...
        doc.save(`Quatation_${safeInvoiceNo}_${safeCustomer}.pdf`);
    }catch(err){

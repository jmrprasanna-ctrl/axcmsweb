...
const witnessEl = document.getElementById("witness_list");
const dudgementEl = document.getElementById("dudgement");
...
function renderPlaintsTable(rows) {
    ...
    (Array.isArray(rows) ? rows : []).forEach((row) => {
        ...
        tr.addEventListener("click", () => {
            window.location.href = `plaint.html?id=${row.id}`;
        });
        tr.innerHTML = `
...
async function loadPlaints() {
    allPlaints = await request("/plaints", "GET");
    renderPlaintsTable(allPlaints);
}
...
async function loadPlaintForEdit() {
    ...
    witnessEl.value = row.witness_list || "";
    dudgementEl.value = row.dudgement || "";
    finishedEl.checked = !!row.finished;
...
if (plaintFormEl) {
    plaintFormEl.addEventListener("submit", async (e) => {
        ...
        const payload = {
            ...
            witness_list: witnessEl.value.trim(),
            dudgement: dudgementEl.value.trim(),
            finished: !!finishedEl.checked,
        };
...
(async function init() {
    try {
        ...
        alert(err.message || "Failed to initialize plaint page.");
    }
})();
